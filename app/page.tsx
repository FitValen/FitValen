...
function Training({ go, start, notify, routines, createRoutine }: { go:(v:View)=>void; start:()=>void; notify:(s:string)=>void; routines:FitValenState["routines"]; createRoutine:()=>void }) {
  ...
  return <><Header eyebrow="PLANIFICA · ENTRENA · MEJORA" title="Entrenamiento" admin={()=>go("admin")}/><div className="tabs"><button className={tab==="rutinas"?"active":""} onClick={()=>setTab("rutinas")}>Mis rutinas</button><button className={tab==="progreso"?"active":""} onClick={()=>setTab("progreso")}>Progreso</button></div>
    {tab==="rutinas"?<><section className="training-lead"><div><span className="tag"><Icon name="fire" size={15}/> HOY</span><h2>Empuje · Pecho y hombro</h2><p>La última vez moviste 5.840 kg. Hoy puedes superarlo.</p><button className="primary" onClick={start}><Icon name="play"/> Empezar ahora</button></div><div className="big-number"><strong>0%</strong><span>0 de 17 series</span></div></section><SectionTitle eyebrow="TU PLAN" title="Rutinas de esta semana" action="+ Crear rutina" click={createRoutine}/><section className="routines">{[["Empuje","Pecho · Hombro · Tríceps","HOY","green","5 ejercicios"],["Tirón","Espalda · Bíceps","VIERNES","blue","6 ejercicios"],["Pierna completa","Cuádriceps · Femoral · Glúteo","DOMINGO","purple","7 ejercicios"],...routines.map(r=>[r.name,r.focus,r.day,"green",`${r.exercises} ejercicios`])].map(r=><article className="card routine" key={`${r[0]}-${r[2]}`}><span className={`round-icon ${r[3]}`}><Icon name="gym"/></span><small>{r[2]}</small><h3>{r[0]}</h3><p>{r[1]}</p><footer><span>{r[4]}</span><button onClick={()=>notify(`${r[0]} abierta`)} aria-label={`Abrir ${r[0]}`}><Icon name="arrow"/></button></footer></article>)}</section></>:<Progress/>}
  </>;
}
...
function Nutrition({go,notify,waterMl,addWater,mealPlans,createMealPlan,addMeal}:{go:(v:View)=>void;notify:(s:string)=>void;waterMl:number;addWater:(amount:number)=>void;mealPlans:FitValenState["mealPlans"];createMealPlan:()=>void;addMeal:()=>void}){return <><Header eyebrow="JUEVES, 13 DE AGOSTO" title="Nutrición" admin={()=>go("admin")}/><section className="nutrition-lead"><div><small>RESUMEN DE HOY</small><h2>Vas por buen camino</h2><p>Te quedan 830 kcal para completar tu objetivo diario.</p></div><div className="nutrition-number"><strong>1.420</strong><span>de 2.250 kcal</span></div></section><section className="macros"><Macro name="Proteína" now="122" total="170 g" value={72}/><Macro name="Carbohidratos" now="148" total="250 g" value={59} blue/><Macro name="Grasas" now="46" total="70 g" value={66} orange/></section><section className="nutrition-grid"><article className="card meals"><SectionTitle eyebrow="REGISTRO DIARIO" title="Tus comidas" action="+ Añadir comida" click={addMeal}/>{[["Desayuno","08:15","Avena con yogur y frutos rojos","420 kcal"],["Comida","14:10","Arroz, pollo y verduras","685 kcal"],["Snack","17:30","Plátano y crema de cacahuete","315 kcal"],["Cena","Pendiente","Sin registrar","—"]].map((m,i)=><div className={`meal ${i===3?"empty":""}`} key={m[0]}><span><Icon name="food"/></span><div><strong>{m[0]} <small>{m[1]}</small></strong><p>{m[2]}</p></div><b>{m[3]}</b><button onClick={i===3?addMeal:()=>notify(`${m[0]} abierto`)} aria-label={`${i===3?"Añadir":"Abrir"} ${m[0]}`}><Icon name={i===3?"plus":"arrow"}/></button></div>)}{mealPlans.map(plan=><div className="meal" key={plan.id}><span><Icon name="apple"/></span><div><strong>{plan.name} <small>PLAN</small></strong><p>{plan.goal} · {plan.meals} comidas</p></div><b>{plan.calories} kcal</b><button onClick={()=>notify(`${plan.name} abierto`)} aria-label={`Abrir ${plan.name}`}><Icon name="arrow"/></button></div>)}<button className="add-set" onClick={createMealPlan}><Icon name="plus"/> Crear dieta o menú</button></article><article className="card hydration"><small>HIDRATACIÓN</small><Icon name="water" size={42}/><strong>{(waterMl/1000).toFixed(2).replace(".",",")} L</strong><span>de 2,5 L</span><Bar value={Math.min(100,waterMl/25)} blue/><div><button onClick={()=>addWater(250)}>+250 ml</button><button onClick={()=>addWater(500)}>+500 ml</button></div><em>Próximo recordatorio: 18:30</em></article></section></>}
...
type ModalKind = "routine" | "diet" | "meal" | "event" | "user" | "settings";

function ActionModal({kind,close,submit}:{kind:ModalKind;close:()=>void;submit:(values:Record<string,string>)=>void}){
  const titles:Record<ModalKind,string>={routine:"Crear rutina",diet:"Crear dieta o menú",meal:"Registrar comida",event:"Programar actividad",user:"Gestionar usuario y permisos",settings:"Configuración general"};
  function save(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget).entries()) as Record<string,string>;submit(values)}
  return <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><section className="action-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><header><div><small>ACCIÓN ADMINISTRABLE</small><h2 id="modal-title">{titles[kind]}</h2></div><button className="icon-btn" onClick={close} aria-label="Cerrar"><Icon name="close"/></button></header><form onSubmit={save}>
    {kind==="routine"&&<><label>Nombre de la rutina<input name="name" placeholder="Ej. Full body" required autoFocus/></label><label>Enfoque<select name="focus" defaultValue="Fuerza"><option>Fuerza</option><option>Hipertrofia</option><option>Resistencia</option><option>Movilidad</option></select></label><div className="form-row"><label>Día<select name="day" defaultValue="LUNES"><option>LUNES</option><option>MARTES</option><option>MIÉRCOLES</option><option>JUEVES</option><option>VIERNES</option><option>SÁBADO</option><option>DOMINGO</option></select></label><label>Ejercicios<input name="exercises" type="number" min="1" max="20" defaultValue="5" required/></label></div></>}
    {kind==="diet"&&<><label>Nombre del plan<input name="name" placeholder="Ej. Definición semanal" required autoFocus/></label><label>Objetivo<select name="goal"><option>Pérdida de grasa</option><option>Ganancia muscular</option><option>Mantenimiento</option><option>Rendimiento</option></select></label><div className="form-row"><label>Calorías diarias<input name="calories" type="number" min="800" max="6000" defaultValue="2200" required/></label><label>Comidas<input name="meals" type="number" min="1" max="10" defaultValue="4" required/></label></div></>}
    {kind==="meal"&&<><label>Tipo de comida<select name="name" autoFocus><option>Desayuno</option><option>Comida</option><option>Snack</option><option>Cena</option></select></label><label>Descripción<input name="description" placeholder="Alimentos consumidos" required/></label><label>Calorías<input name="calories" type="number" min="0" defaultValue="400" required/></label></>}
    {kind==="event"&&<><label>Actividad<input name="title" placeholder="Nombre de la actividad" required autoFocus/></label><label>Tipo<select name="kind"><option value="workout">Entrenamiento</option><option value="meal">Menú</option></select></label><div className="form-row"><label>Día<input name="day" type="date" required/></label><label>Hora<input name="time" type="time" required/></label></div></>}
    {kind==="user"&&<><label>Usuario<select name="user"><option>Daniel Valenzuela</option><option>Usuario 2</option></select></label><label>Rol<select name="role"><option value="admin">Administrador</option><option value="member">Miembro</option></select></label><fieldset><legend>Permisos</legend><label className="check-field"><input type="checkbox" name="training" defaultChecked/> Gestionar rutinas</label><label className="check-field"><input type="checkbox" name="nutrition" defaultChecked/> Gestionar dietas</label><label className="check-field"><input type="checkbox" name="calendar" defaultChecked/> Programar calendario</label></fieldset></>}
    {kind==="settings"&&<><label>Objetivo de agua (ml)<input name="water" type="number" min="500" step="250" defaultValue="2500"/></label><label>Unidades<select name="units"><option>Métricas (kg, cm)</option><option>Imperiales (lb, in)</option></select></label></>}
    <footer><button type="button" className="outline" onClick={close}>Cancelar</button><button className="primary">Guardar cambios</button></footer></form></section></div>
}
...
export default function Page(){
  ...
  async function loadData(currentUser:SessionUser){
    ...
    setData({...payload.state,routines:payload.state.routines??[],mealPlans:payload.state.mealPlans??[],scheduled:payload.state.scheduled??[]});
    setUser(currentUser);
  ...
  const submitModal=(values:Record<string,string>)=>{
    if(!modal)return;
    if(modal==="routine")setData(current=>({...current,routines:[...current.routines,{id:crypto.randomUUID(),name:values.name,focus:values.focus,day:values.day,exercises:Number(values.exercises)}]}));
    if(modal==="diet")setData(current=>({...current,mealPlans:[...current.mealPlans,{id:crypto.randomUUID(),name:values.name,goal:values.goal,calories:Number(values.calories),meals:Number(values.meals)}]}));
    if(modal==="event")setData(current=>({...current,scheduled:[...current.scheduled,{id:crypto.randomUUID(),title:values.title,kind:values.kind as "workout"|"meal",day:values.day,time:values.time}]}));
    const labels:Record<ModalKind,string>={routine:"Rutina creada",diet:"Dieta creada",meal:"Comida registrada",event:"Actividad programada",user:"Permisos actualizados",settings:"Configuración guardada"};
    notify(`${labels[modal]} · guardando…`);setModal(null);
  };
  ...
  return <div className="shell"><aside><button className="brand" onClick={()=>setView("inicio")}><span>FV</span><strong>FitValen</strong></button><nav>{nav.map(x=><button className={view===x.id?"active":""} key={x.id} onClick={()=>setView(x.id)}><Icon name={x.icon}/><span>{x.label}</span>{x.id==="entrenamiento"&&<i/>}</button>)}</nav><div className="aside-bottom">{user?.role==="admin"&&<button className={view==="admin"?"active":""} onClick={()=>setView("admin")}><Icon name="shield"/> Administración</button>}<button onClick={signOut}><Icon name="close"/> Cerrar sesión</button><section><div><Icon name="fire"/><span><strong>{saveStatus==="saving"?"Guardando…":saveStatus==="error"?"Error al guardar":"Datos seguros"}</strong><small>{saveStatus==="saved"?"Guardados en la VPS":"Sesión persistente"}</small></span></div><Bar value={saveStatus==="saved"?100:75}/></section></div></aside><main>{view==="inicio"&&<Home go={setView} start={()=>setWorkout(true)} notify={notify} waterMl={data.hydration.waterMl} addWater={addWater}/>} {view==="entrenamiento"&&<Training go={setView} start={()=>setWorkout(true)} notify={notify} routines={data.routines} createRoutine={()=>setModal("routine")}/>} {view==="calendario"&&<Calendar go={setView} program={()=>setModal("event")}/>} {view==="nutricion"&&<Nutrition go={setView} notify={notify} waterMl={data.hydration.waterMl} addWater={addWater} mealPlans={data.mealPlans} createMealPlan={()=>setModal("diet")} addMeal={()=>setModal("meal")}/>} {view==="perfil"&&<Profile go={setView} notify={notify}/>} {view==="admin"&&user?.role==="admin"&&<Admin go={setView} notify={notify} openManager={setModal}/>}<div className="end-space"/></main><nav className="mobile-nav">{nav.map(x=><button className={view===x.id?"active":""} key={x.id} onClick={()=>setView(x.id)}><Icon name={x.icon}/><small>{x.label}</small></button>)}</nav>{modal&&<ActionModal kind={modal} close={()=>setModal(null)} submit={submitModal}/>} {workout&&<Workout close={()=>setWorkout(false)} notify={notify} sets={data.workout.completedSets} setSets={setSets} weights={data.workout.weightsKg} repetitions={data.workout.repetitions} setNumbers={setNumbers}/>} {toast&&<div className="toast"><Icon name="check"/>{toast}</div>}</div>
}
